...
export async function sendComplaintConfirmationEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'JanSankalp AI <onboarding@resend.dev>', // Use verified domain in production
      to: [email],
...
export async function sendComplaintAssignedEmail(
  email: string,
  name: string,
  ticketId: string,
  officerName: string,
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'JanSankalp AI <onboarding@resend.dev>',
      to: [email],
      subject: `Officer Assigned to your Report – ${ticketId}`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
          <h1 style="color: #1e3a8a; text-transform: uppercase; letter-spacing: 2px;">JanSankalp AI</h1>
          <p style="font-size: 16px; color: #475569;">Namaste <strong>${name}</strong>,</p>
          <p style="font-size: 14px; color: #475569;">An officer has been assigned to investigate your civic report.</p>
          
          <div style="background-color: #f8fafc; padding: 20px; border-radius: 12px; margin: 20px 0; border-left: 4px solid #3b82f6;">
            <p style="margin: 0; font-size: 10px; font-weight: 800; color: #64748b; text-transform: uppercase;">Ticket ID</p>
            <p style="margin: 5px 0 15px 0; font-size: 18px; font-weight: 900; color: #0f172a;">${ticketId}</p>
            
            <p style="margin: 0; font-size: 10px; font-weight: 800; color: #64748b; text-transform: uppercase;">Assigned Officer</p>
            <p style="margin: 5px 0 15px 0; font-size: 16px; font-weight: 900; color: #3b82f6;">${officerName}</p>
            
            <p style="margin: 0; font-size: 10px; font-weight: 800; color: #64748b; text-transform: uppercase;">Report Title</p>
            <p style="margin: 5px 0 0 0; font-size: 14px; font-weight: 600;">${title}</p>
          </div>
          
          <p style="font-size: 14px; color: #475569;">The officer will review the details and provide updates as the work progresses.</p>
          
          <a href="${process.env.NEXTAUTH_URL}/dashboard/my-reports" style="display: inline-block; background-color: #1e3a8a; color: white; padding: 12px 24px; text-decoration: none; border-radius: 99px; font-weight: 800; font-size: 12px; text-transform: uppercase; margin-top: 20px;">Track Live Status</a>
    ...
    console.error("Assignment Email Service Error:", error);
    return { success: false, error };
  }
}
