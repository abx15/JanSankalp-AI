...
export const AIAnimatedIcon = () => {
  ...
  if (!mounted) return <div className="w-24 h-24" />;
...
export const FloatingParticle = ({ delay = 0 }) => {
  const [mounted, setMounted] = React.useState(false);
  const [randomPos, setRandomPos] = React.useState({ left: "50%", top: "50%" });
  const [randomDuration, setRandomDuration] = React.useState(5);
  ...
  React.useEffect(() => {
    ...
    setRandomDuration(5 + Math.random() * 5);
    setMounted(true);
  }, []);
