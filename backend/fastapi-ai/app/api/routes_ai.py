...
async def assistant_response_generator(user_id: str, message: str, role: str, context: Any = None):
    ...
    res = await assistant_service.get_response(
        user_id=user_id,
        message=message,
        role=role,
        context=context
    ...
    # SSE Protocol: yield metadata first
    yield f"data: {json.dumps({'event': 'start', 'assistant_name': res.get('assistant_name', 'JanSahayak'), 'intent': res.get('intent', 'GENERAL_QUERY'), 'sentiment': res.get('sentiment', 'NEUTRAL')})}\n\n"
    ...
    # SSE Protocol: yield final completion signal with suggestions
    yield f"data: {json.dumps({'event': 'end', 'suggested_actions': res.get('suggested_actions', []), 'timestamp': res.get('timestamp')})}\n\n"
...
async def assistant_stream(request: Request):
    ...
    return StreamingResponse(
        assistant_response_generator(user_id, message, role, context),
