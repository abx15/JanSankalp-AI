...
export class WorkflowsService {
  ...
  async getComplaints(filters: any) {
    ...
    if (status) query.where.status = status as ComplaintStatus;
    if (category) query.where.category = category;
    if (severity) query.where.severity = parseInt(severity, 10);
    if (authorId) query.where.authorId = authorId;
    if (assignedToId) query.where.assignedToId = assignedToId;
    if (districtId) query.where.districtId = districtId;
  ...
  async createComplaint(data: any) {
    ...
    // Fetch author details for tenancy mapping
    ...
    // Enqueue automated multi-agent reasoning workflow in background
    try {
    ...
    // Real-time notification trigger (Simulating Pusher/WS gateway trigger)
    try {
  ...
  async assignComplaint(complaintId: string, officerId: string, assignedBy: any) {
    const officer = await this.prisma.user.findUnique({
      where: { id: officerId },
    });

    if (!officer || officer.role !== Role.OFFICER) {
      throw new BadRequestException('Invalid officer selected');
    }
    ...
    // Notify officer and governance channels
    try {
  ...
  async updateComplaint(complaintId: string, data: any, sessionUser: any) {
    ...
    const finalStatus = (aiVerification && !aiVerification.verified) ? ComplaintStatus.IN_PROGRESS : (status as ComplaintStatus);
    ...
    // Award 50 points and send resolution email receipt if transitioned to RESOLVED
    if (finalStatus === ComplaintStatus.RESOLVED && originalComplaint.status !== ComplaintStatus.RESOLVED) {
      // 1. Grant 50 social trust impact points to the citizen (author)
      try {
      ...
      // 2. Send premium civic invoice/receipt HTML email
      ...
      // Calculate automated/simulated SLA performance metric
      ...
      // Premium, beautiful responsive civic invoice/receipt HTML template
      const emailHtml = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="wrapper">
    <div class="container">
      ...
      // Trigger POST request using fetch
      try {
