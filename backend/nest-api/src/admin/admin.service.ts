...
export class AdminService {
  ...
  async getStats() {
    ...
    const totalAdmins = await this.prisma.user.count({ where: { role: Role.ADMIN } });
    const totalOfficers = await this.prisma.user.count({ where: { role: Role.OFFICER } });
    const totalCitizens = await this.prisma.user.count({ where: { role: Role.CITIZEN } });
    ...
    const pendingComplaints = await this.prisma.complaint.count({ where: { status: ComplaintStatus.PENDING } });
    const inProgressComplaints = await this.prisma.complaint.count({ where: { status: ComplaintStatus.IN_PROGRESS } });
    const resolvedComplaints = await this.prisma.complaint.count({ where: { status: ComplaintStatus.RESOLVED } });
    const rejectedComplaints = await this.prisma.complaint.count({ where: { status: ComplaintStatus.REJECTED } });
  ...
  async getAnalytics(user: any) {
    ...
    if (user.role === Role.STATE_ADMIN) tenantFilter.stateId = user.stateId;
    if (user.role === Role.DISTRICT_ADMIN) tenantFilter.districtId = user.districtId;
    if (user.role === Role.CITY_ADMIN) tenantFilter.cityId = user.cityId;
    if (user.role === Role.OFFICER) tenantFilter.districtId = user.districtId;
    ...
    // Dynamic filtering for raw query
    let rawTrends: any[] = [];
    if (user.stateId || user.districtId || user.cityId) {
      const filters: string[] = [];
      if (user.stateId) filters.push(`"stateId" = '${user.stateId}'`);
      if (user.districtId) filters.push(`"districtId" = '${user.districtId}'`);
      if (user.cityId) filters.push(`"cityId" = '${user.cityId}'`);
  ...
  async getAiStats() {
    ...
    const spamRejected = await this.prisma.complaint.count({
      where: { status: ComplaintStatus.REJECTED, spamScore: { gt: 0.5 } },
    });
    const autoRouted = await this.prisma.complaint.count({
      where: { status: ComplaintStatus.IN_PROGRESS, NOT: { assignedToId: null } },
    });
  ...
  async triggerAiProcess() {
    ...
    const AI_SERVICE_URL = this.configService.get<string>('aiService.url');
    const results = [];
  ...
  async createDepartment(data: any) {
    ...
    if (existing) throw new BadRequestException('Department already exists');
  ...
  async updateDepartment(id: string, data: any) {
    ...
    if (conflict) throw new BadRequestException('A department with this name already exists');
  ...
  async deleteDepartment(id: string) {
    ...
    if (dept._count.complaints > 0) {
      throw new BadRequestException(
        `Cannot delete: ${dept._count.complaints} complaints are linked to this department. Reassign them first.`,
      );
    }
